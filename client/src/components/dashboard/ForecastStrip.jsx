        ...
        <MonthBlock key={i} $current={i === 0}>
          ...
          <EnergyRow>
            <EnergyPill $level={month.userA.energyLevel}>
              You {ENERGY_GLYPHS[month.userA.energyLevel]}
            </EnergyPill>
            <EnergyPill $level={month.userB.energyLevel}>
              Them {ENERGY_GLYPHS[month.userB.energyLevel]}
            </EnergyPill>
          </EnergyRow>
          <RiskLine $risk={month.mismatchRisk}>
            {month.mismatchRisk} risk
          </RiskLine>
...
const MonthBlock = styled.div`
  background: ${({ theme, $current }) =>
    $current ? theme.colors.bgElevated : theme.colors.bg};
  border: 1px solid
    ${({ theme, $current }) =>
      $current ? theme.colors.borderLight : theme.colors.border};

...
function checkWithdrawal(profile, connection, currentMonth) {
  ...
  if (
    !isInWindow(
      currentMonth,
      vulnerabilityWindow.startMonth,
      vulnerabilityWindow.endMonth,
    ) ||
    stressBaseline !== "freeze"
  )
    return null;
...
function checkIntensitySeeking(profile, connection, currentMonth) {
  ...
  if (
    neurotransmitters.dopamine !== "low" ||
    !isInWindow(
      currentMonth,
      vulnerabilityWindow.startMonth,
      vulnerabilityWindow.endMonth,
...
function checkOverCommitment(profile, connection, currentMonth) {
  ...
  if (
    lightProfile !== "high-light" ||
    neurotransmitters.serotonin !== "high" ||
  ...
  return {
    ...
    trigger:
      "High-light profile in peak season — over-commitment risk elevated",
...
async function generate(userId, profile, connections) {
  ...
  if (selfNudge) {
    const exists = await CoachingNudge.findOne({
      userId,
      connectionId: null,

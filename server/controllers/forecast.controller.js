...
async function resolveBothProfiles(connection, ownerUserId) {
  ...
  if (connection.connectedUserId) {
    ...
    partnerDerived = derive(dob, birthLocation.lat, birthLocation.lng, survey || {})
  }
...
async function getForecast(req, res) {
  try {
    const connection = await Connection.findOne({ _id: req.params.connectionId, ownerId: req.user._id })
    if (!connection) return res.status(404).json({ error: 'Connection not found' })
    const now   = new Date()
    const month = now.getMonth() + 1
    const year  = now.getFullYear()
    const { ownerDerived, partnerDerived } = await resolveBothProfiles(connection, req.user._id)
    const forecasts = generateRange(ownerDerived, partnerDerived, month, year, 3)
    const saved = await Promise.all(forecasts.map(f =>
...
async function getForecastRange(req, res) {
  try {
    ...
    if (totalMonths < 1 || totalMonths > 12) return res.status(400).json({ error: 'Range must be 1–12 months' })
    const { ownerDerived, partnerDerived } = await resolveBothProfiles(connection, req.user._id)
    const forecasts = generateRange(ownerDerived, partnerDerived, fromMonth, fromYear, totalMonths)
    const saved = await Promise.all(forecasts.map(f =>
      SeasonalForecast.findOneAndUpdate(
        { connectionId: connection._id, month: f.month, year: f.year },
        { ...f, connectionId: connection._id },

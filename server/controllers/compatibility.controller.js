...
async function getReport(req, res) {
  try {
    ...
    const report = await CompatibilityReport.findById(connection.compatibilityReportId)
...
async function regenerateReport(req, res) {
  try {
    ...
    if (connection.connectedUserId) {
      ...
      if (!pp) return res.status(400).json({ error: 'Partner profile missing' })
    ...
    const updated = await CompatibilityReport.findByIdAndUpdate(
      ...
      { scores: report.scores, tiers: report.tiers, archetype: report.archetype, dimensions: report.dimensions, generatedAt: report.generatedAt },
      { new: true }
    )
    res.json(updated)
  } catch (err) { res.status(500).json({ error: err.message }) }
